...
function Blogs() {
  return (
    ...
    //     <div key={index} className='rounded-md shadow-lg shadow-sky-500/50 hover:bg-sky-500 hover:text-black'>
    //      {/* yha py image vala part */}
    //      <div className='imagify bg-white  p-3'>
    //       <img src={blog.image} alt={blog.title} className='rounded-md'/>
    //      </div>
    //      {/* yha py blog ki detail aye gi */}
    //      <p className='catogory p-2'>{blog.category}</p>
    //      <h5 className='title p-2'>{blog.title}</h5>
    //      <p className='description p-2 hover:text-black'>{blog.description}</p>
    //      <button className='font-bold underline hover:cursor-pointer hover:text-sky-500 p-2'>continue reading</button>
    //     </div>
    //   ))}
    //   </div>
    //   </div>
    ...
    {blogs.map((blog,index)=>(
      ...
      >
        ...
        <div className='relative z-10 p-2 transition-colors duration-500 group-hover:text-white'>
          ...
          <p className='description py-2 hover:text-white'>{blog.description}</p>
