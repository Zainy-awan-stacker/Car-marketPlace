        ...
        const handleFilterChange = (checked,value,type) =>{
          ...
          const sortCars = (a,b)=>{
            ...
            const matchesType = (car)=>{
              ...
              return filter.bodyType.some(type => type.toLowerCase()=== car.type.toLowerCase());
            }
            ...
            },[cars,filter,sort,searchQuery,heroDestination])
    ...
    <section className="Section bg-primary px-5 py-10">
      ...
      <div className="sorting-sec flex flex-col mx-5">
       {/* ye ha sorting vala part */}
        ...
        <select value={sort} onChange={(e) =>setSort(e.target.value)} className="rely p-1 bg-primary border ">
        {sortOptions.map((sort,index)=>(
          <option key={index} value={sort} className={""}>{sort}</option>
        ))}
        </select>
       </div>
       ...
       {/* ye ha sorting vala part */}
        ...
        {bodyType.map((type)=>(
          ...
          <input type="checkbox" checked={filter.bodyType.includes(type)} onChange={(e) =>handleFilterChange(e.target.checked, type, "bodyType")} />
          {type}
          </label>
        ))}
       ...
       <div className="range flex flex-col p-2 rounded-md gap-2 mx-5 bg-primary ">
        ...
        {priceRange.map((price)=>(
          ...
          <input type="checkbox" checked={filter.priceRange.includes(price)} onChange={(e) =>handleFilterChange(e.target.checked, price, "priceRange")}/>
          {currency}{price}
          </label>
        ))}
